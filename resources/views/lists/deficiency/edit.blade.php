@php($title = 'Edit Deficiency Type')
<x-layouts.app :title="$title">
  <div style="max-width:720px;margin:20px auto;">
    <h2>{{ $title }}</h2>
    <form method="post" 
          action="{{ route('lists.deficiency.update', $item) }}"
          class="edit-form"
          onsubmit="return confirmSaveChanges('deficiency type', '{{ $item->name }}')">
      @csrf @method('put')
      ...
      <div style="margin-top:12px;display:flex;gap:8px;">
        <a href="{{ route('lists.deficiency.index') }}" 
           class="cancel-link"
