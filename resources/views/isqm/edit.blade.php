...
<x-layouts.app :title="$title">
  ...
  @php
    // Helper function to parse bullet-pointed text into array
    function parseBulletPoints($text) {
      if (empty($text)) return [''];
      // Split by bullet points (• or -)
      $items = preg_split('/\n\n•\s*|\n\n-\s*/', $text);
      // Remove leading bullet from first item if present
      if (!empty($items[0])) {
        $items[0] = preg_replace('/^•\s*|^-\s*/', '', $items[0]);
      }
    ...
    $qualityObjectiveItems = parseBulletPoints(old('quality_objective', $entry->quality_objective));
    $qualityRiskItems = parseBulletPoints(old('quality_risk', $entry->quality_risk));
    $assessmentItems = parseBulletPoints(old('assessment_of_risk', $entry->assessment_of_risk));
    $responseItems = parseBulletPoints(old('response', $entry->response));
    $implementationItems = parseBulletPoints(old('firm_implementation', $entry->firm_implementation));
    $tocItems = parseBulletPoints(old('toc', $entry->toc));
    $findingsItems = parseBulletPoints(old('findings', $entry->findings));
    $rootCauseItems = parseBulletPoints(old('root_cause', $entry->root_cause));
    $remedialItems = parseBulletPoints(old('remedial_actions', $entry->remedial_actions));
  @endphp

  <div style="max-width:1400px; margin:0 auto; padding:0 12px;">
    <div class="topbar" style="margin-bottom:24px;">
      <h2 style="margin:0;">Edit ISQM Entry #{{ $entry->id }}</h2>
      <div style="display:flex;gap:8px;">
        <a href="{{ route('isqm.show', ['isqm' => $entry, 'return_to' => $returnTo]) }}" class="btn" style="background:#64748b;">View</a>
    ...
    <form method="post" 
          ...
          class="edit-form"
          onsubmit="return confirmSaveChanges(this, 'entry #{{ $entry->id }}')">
      @csrf
      @method('put')
      ...
      <div class="form-section">
          ...
          <div>
            ...
            <select name="area" required>
              @foreach ($areas as $key => $label)
                <option value="{{ $key }}" @selected(old('area', $entry->area) === $key)>{{ $label }}</option>
              @endforeach
            </select>
          </div>
          <div>
            ...
            <select name="status" required>
              <option value="open" @selected(old('status', $entry->status) === 'open')>Open</option>
              <option value="monitoring" @selected(old('status', $entry->status) === 'monitoring')>Monitoring</option>
              <option value="closed" @selected(old('status', $entry->status) === 'closed')>Closed</option>
            </select>
          </div>
        </div>
        ...
        <div class="dynamic-field-group" data-field-name="quality_objective">
          ...
          <div class="dynamic-fields-container">
            @foreach($qualityObjectiveItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="quality_objective[]" rows="4" {{ $index === 0 ? 'required' : '' }}>{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
      ...
      <div class="form-section">
        ...
        <div class="dynamic-field-group" data-field-name="quality_risk">
          ...
          <div class="dynamic-fields-container">
            @foreach($qualityRiskItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="quality_risk[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="dynamic-field-group" data-field-name="assessment_of_risk">
          ...
          <div class="dynamic-fields-container">
            @foreach($assessmentItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="assessment_of_risk[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="form-row-3">
          <div>
            ...
            <select name="likelihood">
              <option value="">—</option>
              <option value="1" @selected(old('likelihood', $entry->likelihood) === true || old('likelihood') === '1')>Yes</option>
              <option value="0" @selected(old('likelihood') !== null && old('likelihood') !== '1' && $entry->likelihood === false)>No</option>
            </select>
          </div>
          <div>
            ...
            <select name="adverse_effect">
              <option value="">—</option>
              <option value="1" @selected(old('adverse_effect', $entry->adverse_effect) === true || old('adverse_effect') === '1')>Yes</option>
              <option value="0" @selected(old('adverse_effect') !== null && old('adverse_effect') !== '1' && $entry->adverse_effect === false)>No</option>
            </select>
          </div>
          <div>
            ...
            <select name="risk_applicable">
              <option value="">—</option>
              <option value="1" @selected(old('risk_applicable', $entry->risk_applicable) === true || old('risk_applicable') === '1')>Yes</option>
              <option value="0" @selected(old('risk_applicable') !== null && old('risk_applicable') !== '1' && $entry->risk_applicable === false)>No</option>
            </select>
          </div>
        </div>
        ...
        <div class="dynamic-field-group" data-field-name="response">
          ...
          <div class="dynamic-fields-container">
            @foreach($responseItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="response[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="dynamic-field-group" data-field-name="firm_implementation">
          ...
          <div class="dynamic-fields-container">
            @foreach($implementationItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="firm_implementation[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="dynamic-field-group" data-field-name="toc">
          ...
          <div class="dynamic-fields-container">
            @foreach($tocItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="toc[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
      ...
      <div class="form-section">
          ...
          <div>
            ...
            <select name="monitoring_activity_id">
              ...
              @foreach ($monitoringActivities as $m)
                <option value="{{ $m->id }}" @selected(old('monitoring_activity_id', $entry->monitoring_activity_id) == $m->id)>{{ $m->name }}</option>
              @endforeach
            </select>
          </div>
          <div>
            ...
            <select name="deficiency_type_id">
              ...
              @foreach ($deficiencyTypes as $d)
                <option value="{{ $d->id }}" @selected(old('deficiency_type_id', $entry->deficiency_type_id) == $d->id)>{{ $d->name }}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="dynamic-field-group" data-field-name="findings">
          ...
          <div class="dynamic-fields-container">
            @foreach($findingsItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="findings[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="dynamic-field-group" data-field-name="root_cause">
          ...
          <div class="dynamic-fields-container">
            @foreach($rootCauseItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="root_cause[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
        ...
        <div class="checkbox-group">
          <label><input type="checkbox" name="severe" value="1" @checked(old('severe', $entry->severe))> Severe</label>
          <label><input type="checkbox" name="pervasive" value="1" @checked(old('pervasive', $entry->pervasive))> Pervasive</label>
          <label><input type="checkbox" name="objective_met" value="1" @checked(old('objective_met', $entry->objective_met))> Objective Met</label>
        </div>
        ...
        <div class="dynamic-field-group" data-field-name="remedial_actions">
          ...
          <div class="dynamic-fields-container">
            @foreach($remedialItems as $index => $item)
              <div class="dynamic-field-item">
                <textarea name="remedial_actions[]" rows="4">{{ $item }}</textarea>
                @if($index > 0)
                  <button type="button" class="btn-remove-field" style="margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
                @else
                  <button type="button" class="btn-remove-field" style="display:none;margin-top:8px;padding:6px 12px;background:#ef4444;color:white;border:none;border-radius:4px;cursor:pointer;font-size:12px;">Remove</button>
      ...
      <div class="form-section">
          ...
          <div>
            ...
            <select name="client_id">
              ...
              @foreach ($clients as $c)
                <option value="{{ $c->id }}" @selected(old('client_id', $entry->client_id) == $c->id)>{{ $c->name }}</option>
              @endforeach
            </select>
          </div>
          <div>
            ...
            <select name="owner_id">
              ...
              @foreach ($users as $u)
                <option value="{{ $u->id }}" @selected(old('owner_id', $entry->owner_id) == $u->id)>{{ $u->name }}</option>
              @endforeach
            </select>
          </div>
        </div>
        ...
        <div class="form-row">
          <div>
            ...
            <input type="date" name="due_date" value="{{ old('due_date', optional($entry->due_date)->format('Y-m-d')) }}">
          </div>
          <div>
            <label>Review Date</label>
            <input type="date" name="review_date" value="{{ old('review_date', optional($entry->review_date)->format('Y-m-d')) }}">
          </div>
        </div>

        <div class="checkbox-group">
          <label><input type="checkbox" name="entity_level" value="1" @checked(old('entity_level', $entry->entity_level))> Entity Level</label>
          <label><input type="checkbox" name="engagement_level" value="1" @checked(old('engagement_level', $entry->engagement_level))> Engagement Level</label>
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        @endif
        <label>Upload Additional Files (Multiple files allowed)</label>
        <input type="file" name="files[]" id="file-input-edit" multiple accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" style="margin-bottom:8px;">
        <div id="file-list-edit" style="margin-top:8px;padding:8px;background:#f8fafc;border-radius:6px;display:none;">
          <strong style="display:block;margin-bottom:4px;font-size:13px;">Selected files:</strong>
          <ul id="file-names-edit" style="list-style:none;padding:0;margin:0;font-size:13px;color:#475569;"></ul>
        </div>
      ...
      <div class="form-actions">
        <button type="submit" class="btn save-btn">Update Entry</button>
        <a href="{{ route('isqm.show', ['isqm' => $entry, 'return_to' => $returnTo]) }}" class="btn" style="background:#64748b;">View</a>
        <a href="{{ $returnTo }}" 
  ...
  <script>
    // Dynamic field management (same as create form)
      ...
      document.querySelectorAll('.btn-add-field').forEach(button => {
        button.addEventListener('click', function() {
          ...
          if (firstTextarea.hasAttribute('required')) {
            newTextarea.removeAttribute('required');
          }
    ...
    function confirmSaveChanges(form, itemName) {
    ...
    const fileNamesEdit = document.getElementById('file-names-edit');
    
    if (fileInputEdit) {
      fileInputEdit.addEventListener('change', function(e) {
        const files = Array.from(e.target.files);
        if (files.length > 0) {
          fileNamesEdit.innerHTML = '';
          files.forEach((file, index) => {
            const li = document.createElement('li');
            li.style.padding = '4px 0';
            li.innerHTML = `<span style="color:#059669;">✓</span> ${file.name} <span style="color:#64748b;">(${(file.size / 1024).toFixed(2)} KB)</span>`;
            fileNamesEdit.appendChild(li);
          });
          fileListEdit.style.display = 'block';
        } else {
          fileListEdit.style.display = 'none';
        }
      });
    }
