...
<x-layouts.app :title="$title">
  <div
    ...
    data-module-endpoint="{{ route('api.modules.show', $module) }}"
    data-lists-endpoint="{{ route('api.lists') }}"
    data-view-url="{{ url('isqm') }}/:id"
    data-edit-url="{{ url('isqm') }}/:id/edit"
  >
    <nav style="font-size:13px;color:#64748b;margin-bottom:12px;display:flex;gap:6px;align-items:center;">
      <a href="{{ route('dashboard') }}" style="color:var(--brand-blue);text-decoration:none;">Dashboard</a>
      <span>/</span>
      <span>{{ $title }}</span>
    </nav>
    ...
    <div class="topbar">
      <h2>{{ $title }}</h2>
      <a class="btn" href="{{ route('isqm.create', ['area' => str_replace('-', '_', $module->slug), 'return_to' => request()->fullUrl()]) }}">Add Entry</a>
    </div>

    @if($module->quality_objective)
      <div style="background:#0f172a;color:#e2e8f0;border-radius:12px;padding:24px;margin-bottom:20px;">
        ...
        <p style="margin:0;font-size:16px;line-height:1.6;color:#f8fafc;white-space:pre-wrap;">{{ $module->quality_objective }}</p>
      </div>
    @endif
    ...
    <section class="filter-section">
      ...
      <form method="get" action="{{ request()->url() }}" id="areaFilters" data-module-form class="filter-form">
        <div class="filter-search-row">
          <div class="filter-field">
            <label class="filter-label" for="search-input">Search</label>
            <input type="text" id="search-input" name="search" value="{{ request('search') }}" placeholder="Search objectives, risks..." class="filter-input filter-search">
          </div>
          <div class="filter-field">
          ...
          <div class="filter-actions">
            ...
            <a href="{{ request()->url() }}" class="btn btn-secondary" data-module-clear>Clear</a>
          </div>
        </div>
          ...
          @foreach($filterFields as $field)
            <div class="filter-field">
              ...
              @if($field['type'] === 'select')
                <select name="{{ $field['name'] }}" class="filter-input">
                  @if($field['includeAll'] ?? false)
                    <option value="">All</option>
                  @endif
                  @if($field['isAssoc'] ?? false)
                    @foreach($field['options'] as $id => $name)
                      <option value="{{ $id }}" @selected(($filters[$field['key']] ?? '') == $id)>{{ $name }}</option>
                    @endforeach
                  @else
                    @foreach($field['options'] as $value => $label)
                      <option value="{{ $value }}" @selected(($filters[$field['key']] ?? '') === $value)>{{ $label }}</option>
                    @endforeach
                  @endif
                </select>
              @elseif($field['type'] === 'date')
                <input type="date" name="{{ $field['name'] }}" value="{{ $field['value'] }}" class="filter-input">
              @endif
            </div>
          @endforeach
        </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:24px;">
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:var(--brand-blue);margin-top:4px;" data-module-stat="total">{{ $stats['total'] }}</div>
    </div>
    <div style="background:#fef3c7;border:1px solid #fde047;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:#92400e;margin-top:4px;" data-module-stat="open">{{ $stats['open'] }}</div>
    </div>
    <div style="background:#dbeafe;border:1px solid #93c5fd;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:#1e40af;margin-top:4px;" data-module-stat="monitoring">{{ $stats['monitoring'] }}</div>
    </div>
    <div style="background:#d1fae5;border:1px solid #86efac;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:#065f46;margin-top:4px;" data-module-stat="closed">{{ $stats['closed'] }}</div>
    </div>
    <div style="background:#fee2e2;border:1px solid #fecaca;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:#b91c1c;margin-top:4px;" data-module-stat="severe">{{ $stats['severe'] }}</div>
    </div>
    <div style="background:#ede9fe;border:1px solid #c4b5fd;border-radius:10px;padding:16px;">
      ...
      <div style="font-size:28px;font-weight:700;color:#5b21b6;margin-top:4px;" data-module-stat="pervasive">{{ $stats['pervasive'] }}</div>
    </div>
    <div style="background:#fee2e2;border:1px solid #fecaca;border-radius:10px;padding:16px;">
      <div style="font-size:12px;color:#991b1b;text-transform:uppercase;letter-spacing:0.08em;">Overdue</div>
      <div style="font-size:28px;font-weight:700;color:#991b1b;margin-top:4px;" data-module-stat="overdue">{{ $stats['overdue'] }}</div>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:16px;margin-bottom:28px;">
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px;">
        ...
        <ul style="margin:0;padding-left:18px;font-size:13px;color:#475569;" data-module-monitoring>
          @forelse($monitoringBreakdown as $item)
            <li style="margin-bottom:6px;">{{ $item->name }} <span style="color:#1e40af;font-weight:600;">×{{ $item->total }}</span></li>
          @empty
      ...
      <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:20px;">
        ...
        <ul style="margin:0;padding-left:18px;font-size:13px;color:#475569;" data-module-deficiency>
          @forelse($deficiencyBreakdown as $item)
            <li style="margin-bottom:6px;">{{ $item->name }} <span style="color:#b45309;font-weight:600;">×{{ $item->total }}</span></li>
          @empty
    ...
    <div class="isqm-table-wrap" style="max-height:calc(100vh - 320px);overflow-y:auto;overflow-x:auto;background:#fff;border:1px solid #e2e8f0;border-radius:8px;" role="region" aria-label="ISQM entries table" tabindex="0">
      <table style="width:100%;border-collapse:collapse;font-size:13px;min-width:2200px;" role="table" aria-label="ISQM entries with filters">
        ...
        <tbody data-module-table role="rowgroup">
          {{-- Table will be populated by JavaScript via AJAX --}}
          <tr role="row">
          ...
          @forelse ($entries as $e)
            <tr style="border-bottom:1px solid #e2e8f0;display:none;" data-server-rendered>
              <td style="padding:12px 8px;vertical-align:top;color:#475569;">{{ $e->import_source ?? ('#' . $e->id) }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">
                <div style="font-weight:600;color:var(--brand-blue);margin-bottom:4px;">{{ $e->quality_objective }}</div>
                <div style="display:flex;gap:8px;margin-top:12px;">
                  ...
                  <a href="{{ route('isqm.edit', ['isqm' => $e, 'return_to' => request()->fullUrl()]) }}" class="btn" style="padding:6px 10px;font-size:12px;">Edit</a>
                </div>
              </td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->quality_risk ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->assessment_of_risk ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->likelihood === true ? 'pill-yes' : ($e->likelihood === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->likelihood === true ? 'Yes' : ($e->likelihood === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->adverse_effect === true ? 'pill-yes' : ($e->adverse_effect === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->adverse_effect === true ? 'Yes' : ($e->adverse_effect === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                @php
                ...
                @endphp
                <div>
                  <span class="status-pill {{ $riskApplicableClass }}">{{ $riskApplicableLabel }}</span>
                  @if(!empty($riskApplicableDetails))
                    <div style="margin-top:8px;font-size:12px;color:#475569;white-space:pre-wrap;line-height:1.5;">{{ $riskApplicableDetails }}</div>
                  @endif
                </div>
              </td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->response ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->firm_implementation ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->toc ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">{{ $e->monitoringActivity?->name ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->findings ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">{{ $e->deficiencyType?->name ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->root_cause ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->severe === true ? 'pill-yes' : ($e->severe === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->severe === true ? 'Yes' : ($e->severe === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->pervasive === true ? 'pill-yes' : ($e->pervasive === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->pervasive === true ? 'Yes' : ($e->pervasive === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;white-space:pre-wrap;line-height:1.5;">{{ $e->remedial_actions ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->objective_met === true ? 'pill-yes' : ($e->objective_met === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->objective_met === true ? 'Yes' : ($e->objective_met === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->entity_level === true ? 'pill-yes' : ($e->entity_level === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->entity_level === true ? 'Yes' : ($e->entity_level === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                <span class="status-pill {{ $e->engagement_level === true ? 'pill-yes' : ($e->engagement_level === false ? 'pill-no' : 'pill-na') }}">
                  {{ $e->engagement_level === true ? 'Yes' : ($e->engagement_level === false ? 'No' : '—') }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                {{ $e->implementation_status ? str_replace('_', ' ', ucfirst($e->implementation_status)) : '—' }}
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                @php
                ...
                @endphp
                <span style="font-size:12px;padding:4px 8px;border-radius:12px;display:inline-block;font-weight:500;{{ $statusStyle }}">
                  {{ ucfirst($e->status) }}
                </span>
              </td>
              <td style="padding:12px 8px;vertical-align:top;">{{ $e->owner?->name ?? '—' }}</td>
              <td style="padding:12px 8px;vertical-align:top;">{{ $e->client?->name ?? '—' }}</td>
              @php
              ...
              @endphp
              <td style="padding:12px 8px;vertical-align:top;">
                {!! $dueDateLabel ?? '<span style="color:#94a3b8;">—</span>' !!}
              </td>
              <td style="padding:12px 8px;vertical-align:top;">
                {!! $reviewDateLabel ? e($reviewDateLabel) : '<span style="color:#94a3b8;">—</span>' !!}
              </td>
            </tr>
          @empty
            <tr role="row">
              <td colspan="25" style="padding:40px;text-align:center;color:#64748b;" role="gridcell" aria-colspan="25">
                No entries found in {{ $title }}. <a href="{{ route('isqm.create', ['area' => str_replace('-', '_', $module->slug)]) }}" style="color:var(--brand-blue);" aria-label="Create your first entry in {{ $title }}">Create your first entry</a>
              </td>
            </tr>
          @endforelse
        </tbody>
